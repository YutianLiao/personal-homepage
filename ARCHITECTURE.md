...
`config.ts` → `themeConfig.nav`。Interest Journey / Learning / Demo 使用 `GalleryNavGroup.vue`：名称本身是 Gallery 总览链接，悬浮或聚焦时显示子页菜单。三个分区的数据统一来自 `gallery-sections.ts`，总览由 `SectionGallery.vue` 渲染；截图位于 `docs/public/gallery/`。全站导航中的古人读书背影与两行横渠四句由 `NavMissionVerse.vue` 注入 `Layout.vue` 的 `nav-bar-content-before` slot。
