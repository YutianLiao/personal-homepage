...
`config.ts` → `themeConfig.nav`。Learning / Demo / Interest Journey 下拉项来自各自注册或硬编码子链。全站导航中的古人读书背影与两行横渠四句由 `theme/components/NavMissionVerse.vue` 注入 `Layout.vue` 的 `nav-bar-content-before` slot。
