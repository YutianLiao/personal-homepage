...
`config.ts` → `themeConfig.nav`。Learning / Demo / Interest Journey 下拉项来自各自注册或硬编码子链。首页站名旁的横渠四句题签由 `theme/components/NavMissionVerse.vue` 注入 `Layout.vue` 的 `nav-bar-title-after` slot。
