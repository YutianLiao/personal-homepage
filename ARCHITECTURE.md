...
源目录内的 `README.md`（开发说明）会一并同步到 `docs/{id}/`，但经 `config.ts` 的 `srcExclude: ["**/README.md"]` 排除，不生成公开路由。
...
注册：组件在 `theme/index.ts` 以 `defineAsyncComponent` 注册全局标签（如 `demo-tokenizer`）；页面 `docs/demos/{id}/index.md` 内嵌 `<demo-xxx />`（路由即目录路径）；顶栏与 `/demos/` Gallery 的标题、链接、截图由 `gallery-sections.ts` 驱动。
...
与 Learning 差异：Demo 为交互式 Vue 组件；Sudoku 题库由 `npm run prepare:sudoku` 生成（`predev`/`prebuild` 自动执行）。Demo 详情页 `sidebar: false`、无模块顶栏；顶栏入口与 `/demos/` Gallery 的文案、链接和截图统一由 `gallery-sections.ts` 驱动。
...
新增 Demo：建 `demos/{id}/README.md`（说明）→ 在 `theme/index.ts` 用 `defineAsyncComponent` 注册组件标签 → 建 `docs/demos/{id}/index.md`（`layout: page`、`pageClass: demo-page`、`outline: false`、内嵌 `<demo-xxx />`）→ 在 `gallery-sections.ts` 添加 Gallery 项并将页面截图放入 `docs/public/gallery/demos/`。
...
SVG / WebP / PNG 见 `docs/public/decorative/README.md`，均为轻量素材并**已提交至 Git**（GitHub Pages 从仓库构建，未跟踪文件不会上线）；缺失时组件 `onerror` 隐藏，站点仍可构建。
...
见 [`DESIGN.md`](DESIGN.md) § 响应式策略：桌面 fluid + 最小宽度兜底。实现：`tokens.css` 的 `--site-min-width` / `--site-page-max`，以及 `custom.css` 中的桌面 chrome 覆盖（无整页 zoom 外壳）。
...
- [ ] 新 Demo 已在 `theme/index.ts` 注册组件并建 `docs/demos/{id}/index.md` 页
...
| Demo 组件注册 | `docs/.vitepress/theme/index.ts` |
