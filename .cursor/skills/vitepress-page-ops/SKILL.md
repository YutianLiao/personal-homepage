...
3. **Gallery 导航要同步**:Interest Journey / Learning / Demo 的顶栏与照片总览统一读 `gallery-sections.ts`;新增或删除子页时同步该文件与 `docs/public/gallery/` 截图。Learning 的模块顶栏与 Prev/Next 由构建从模块注册表自动生成;Demo 无 sidebar,组件在 `theme/index.ts` 注册。
...
| --- | --- | --- | --- | --- |
| Learning 文章 | `/hello-agent/*` | `learning/{module}/**/*.md` | `docs/{module}/` | `learning-modules.json`(模块级);顶栏/分页自动生成;Gallery 见 `gallery-sections.ts` |
| My Notes 笔记 | `/my-notes/*` | `learning/my-notes/NN-topic.md` + `index.md` | `docs/my-notes/` | `learning-modules.json` 的 `flat: true`;顶栏/分页自动生成;模块 Gallery 见 `gallery-sections.ts` |
| Demo | `/demos/*` | `demos/{id}/README.md` + `docs/demos/{id}/index.md` | — | `theme/index.ts` 异步注册组件;页面内嵌 `<demo-xxx/>`;标题 / Gallery 见 `gallery-sections.ts` |
...
| Blog | `/blog/*` | `docs/blog/*.md`(含 `index.md`) | — | `config.ts` 的 `nav` 手写 `/blog/` 入口(全站 `sidebar: false`);文章经 `docs/blog/index.md` 列表手动登记 |
...
- **Demo**:建 `demos/{id}/README.md` → 在 `theme/index.ts` 用 `defineAsyncComponent` 注册组件标签(如 `demo-xxx`)→ 建 `docs/demos/{id}/index.md`(`layout: page`、`pageClass: demo-page`、内嵌 `<demo-xxx/>`)→ 在 `gallery-sections.ts` 加卡片与截图。
...
- **Blog 文章**:建 `docs/blog/{YYYY-MM-slug}.md`(frontmatter 写 `title`/`date`)→ 在 `docs/blog/index.md` 按年份列表手动加链接(**唯一登记入口**,无 sidebar)。
...
3. **退注册**:整模块从 `learning-modules.json` 移除;整 Demo 删 `theme/index.ts` 注册、`docs/demos/{id}/` 页面、`demos/{id}/` 源与组件文件。
4. **清导航**:Gallery 子页从 `gallery-sections.ts` 与 `docs/public/gallery/` 移除;Blog 文章从 `docs/blog/index.md` 列表移除;顶层单页从 `config.ts` 手写 `nav` 移除。
...
- [ ] 新增/删除的注册已到位(Learning: `learning-modules.json`;Demo: `theme/index.ts` 组件注册)
- [ ] My Notes 新增/删除笔记已同步 `learning/my-notes/index.md`
- [ ] Gallery 子页已同步 `gallery-sections.ts` 与 `docs/public/gallery/` WebP 截图
- [ ] 手写导航项已增删(顶层单页 `config.ts` nav;Blog 文章 `docs/blog/index.md` 列表)
